...
export const Nis2: React.FC = () => {
  ...
  const isOverdue = (m: Nis2Measure) =>
    !!m.deadline && new Date(m.deadline) < today && m.implementation_status !== 'implemented';
  ...
  const filtered = useMemo(() => {
    return measures.filter(m => {
      ...
      if (search) {
        ...
        if (
          !m.article_ref.toLowerCase().includes(q) &&
    ...
    <Card>
      <CardBody>
        <div className="py-16 text-center">
          ...
          <p className="text-gray-500 dark:text-slate-400 font-medium">Noch keine NIS-2-Maßnahmen vorhanden</p>
          <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">
            Lade den integrierten NIS-2-Katalog (Art. 21), um mit der Umsetzungsverfolgung zu starten.
          </p>
          {canManage && (
            <Button onClick={seed} disabled={seeding} className="mt-4">
              <Download size={16} />{seeding ? 'Lade Katalog...' : 'NIS-2-Katalog laden'}
            </Button>
          )}
        </div>
      </CardBody>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold dark:text-white flex items-center gap-2">
            <AlertOctagon size={24} className="text-blue-600" />NIS-2 · Art. 21 Sicherheitsmaßnahmen
          </h1>
          <p className="text-gray-500 dark:text-slate-400 text-sm">
            Network and Information Security Directive 2 · {measures.length} Maßnahmen
          </p>
        </div>
      </div>
          ...
          <CardBody className="py-4">
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-lg bg-blue-100 dark:bg-blue-900/30 shrink-0">
                <AlertOctagon size={16} className="text-blue-600 dark:text-blue-400" />
              </div>
              <div>
                <p className="font-semibold text-sm dark:text-white">Wesentliche Einrichtungen (Anhang I)</p>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                  Energie, Transport, Bankwesen, Finanzmarktinfrastruktur, Gesundheitswesen, Trinkwasser,
                  Abwasser, digitale Infrastruktur, IKT-Dienstleistungen, Weltraum, öffentliche Verwaltung.
                </p>
              </div>
            </div>
          </CardBody>
        ...
        <Card>
          <CardBody className="py-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-sm dark:text-white">Wichtige Einrichtungen (Anhang II)</p>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                  Post- und Kurierdienste, Abfallwirtschaft, Chemie, Lebensmittel, verarbeitendes Gewerbe,
                  digitale Dienste, Forschung. Alle Art.-21-Maßnahmen gelten für beide Kategorien.
                </p>
              </div>
            </div>
          </CardBody>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'In Umsetzung', value: stats.inProgress, color: 'bg-yellow-500', icon: AlertOctagon },
      ...
      <FilterBar
        search={search}
        onSearch={setSearch}
        searchPlaceholder="Artikel-Referenz, Titel oder Kategorie suchen..."
      ...
      {grouped.map(([category, items]) => {
        ...
        return (
          <Card key={category} className="overflow-hidden">
            ...
            >
              <span className={`text-xs font-bold px-2 py-0.5 rounded shrink-0 ${colorClass}`}>
                {category}
              </span>
              <span className="text-xs text-gray-500 dark:text-slate-400 shrink-0">{items.length} Maßnahmen</span>
                      ...
                      return (
                        <React.Fragment key={m.id}>
                          <tr className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                            <Td>
                              <span className="font-mono text-xs text-gray-500 dark:text-slate-400 whitespace-nowrap">
                                {m.article_ref}
                              </span>
                            </Td>
                            <Td>
                              <div className="flex items-start gap-2">
                                {m.description && (
                                  <button
                                    ...
                                    className="mt-0.5 p-0.5 rounded text-gray-400 hover:text-blue-600 transition-colors shrink-0"
                                ...
                                <div>
                                  ...
                                  {m.responsible && (
                                    <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">{m.responsible.name}</p>
                                  )}
                                </div>
                              </div>
                            </Td>
                            <Td>
                              <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${statusColors[m.implementation_status]}`}>
                                {statusLabels[m.implementation_status]}
                              </span>
                            </Td>
                            <Td>
                              {m.deadline ? (
                                <span className={`text-xs font-medium ${overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-slate-400'}`}>
                                  {format(new Date(m.deadline), 'dd.MM.yyyy')}
                                  {overdue && ' ⚠'}
                                </span>
                              ) : (
                            ...
                            <Td className="text-gray-500 dark:text-slate-400 text-xs">
                              {m.last_review_date
                                ? format(new Date(m.last_review_date), 'dd.MM.yyyy')
                                : '–'}
                            </Td>
                            <Td>
                              {canWrite && (
                          ...
                          {expanded && m.description && (
                            <tr className="bg-gray-50 dark:bg-slate-800/30">
                              ...
                              <td colSpan={5} className="px-4 py-3">
                                ...
                                {m.evidence && (
                                  <div className="mt-2">
                                    <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-slate-500">Nachweis: </span>
                                    <span className="text-xs text-gray-600 dark:text-slate-400">{m.evidence}</span>
                                  </div>
                                )}
                              </td>
                            </tr>
        ...
        <Card>
          <CardBody>
            <div className="py-12 text-center">
              <AlertOctagon size={36} className="mx-auto mb-3 text-gray-300 dark:text-slate-600" />
              <p className="text-gray-500 dark:text-slate-400">Keine Maßnahmen entsprechen den Filterkriterien.</p>
            </div>
          </CardBody>
        </Card>
      )}
      ...
      <Modal
        open={!!editMeasure}
        onClose={() => setEditMeasure(null)}
        title={editMeasure ? `${editMeasure.article_ref} – ${editMeasure.title}` : ''}
        size="lg"
      >
        <form onSubmit={saveEdit} className="space-y-4">
          {editMeasure && (
            <div className="flex gap-2 flex-wrap">
              <span className="text-[11px] px-2 py-0.5 rounded-full font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                {editMeasure.category}
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Frist (Deadline)</label>
              <input
                type="date"
          ...
          <div className="flex gap-3 pt-2">
            <Button type="button" variant="secondary" onClick={() => setEditMeasure(null)} className="flex-1 justify-center">
              Abbrechen
            </Button>
            {canWrite && (
