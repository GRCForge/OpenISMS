...
export const Topology: React.FC = () => {
  ...
  const typeLabels: Record<string, string> = {
    hardware: t('types.hardware'),
    software: t('types.software'),
    information: t('types.information'),
    process: t('types.process'),
    service: t('types.service'),
    personal: t('types.personal'),
    application: t('types.application'),
    data: t('types.data'),
    ai_application: t('types.ai_application'),
    ai_agent: t('types.ai_agent'),
    other: t('types.other'),
  };
  ...
  if (loading) return (
    <div className="space-y-6" role="status" aria-label={t('loadingLabel')}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { key: 'assets', icon: Box, label: t('stats.assets'), value: stats.total, color: 'bg-blue-500' },
          { key: 'roots', icon: GitBranch, label: t('stats.roots'), value: stats.roots, color: 'bg-indigo-500' },
          { key: 'linked', icon: Layers, label: t('stats.linked'), value: stats.linked, color: 'bg-green-500' },
          { key: 'isolated', icon: Network, label: t('stats.isolated'), value: stats.isolated, color: 'bg-gray-400' },
        ].map(s => (
          <Card key={s.key}>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2"><Network size={18} className="text-blue-500" /><h2 className="font-semibold dark:text-white">{t('graph.title')}</h2></div>
            <div className="flex flex-wrap items-center gap-3">
              <Legend color="bg-red-400" label={t('graph.legend.critical')} />
              <Legend color="bg-orange-400" label={t('graph.legend.high')} />
              <Legend color="bg-yellow-400" label={t('graph.legend.medium')} />
              <Legend color="bg-green-400" label={t('graph.legend.low')} />
              <Legend color="bg-slate-300" label={t('graph.legend.unassessed')} />
              <Legend color="bg-violet-400" label={t('graph.legend.ai')} />
            </div>
          </div>
        </CardHeader>
        <CardBody>
