...
export const Input: React.FC<InputProps> = ({ label, error, className = '', ...props }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <input
        className={`px-3 py-2 bg-white dark:bg-slate-800 border rounded-lg text-sm text-gray-900 dark:text-slate-100 focus:outline-hidden focus:ring-2 transition-all placeholder:text-gray-400 dark:placeholder:text-slate-500 ${
          error
            ? 'border-red-500 dark:border-red-500 focus:ring-red-500/20 focus:border-red-500'
            : 'border-gray-300 dark:border-slate-700 focus:ring-blue-500/20 focus:border-blue-500 dark:focus:border-blue-400'
        } ${className}`}
        {...props}
        id={inputId}
