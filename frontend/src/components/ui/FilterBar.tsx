...
// Einheitliche Filterleiste: Suche + Filter in EINER Reihe (wrappt erst bei
// echter Platznot). Verhindert das vertikale Stapeln der Filter.
export const FilterBar: React.FC<FilterBarProps> = ({
  search, onSearch, searchPlaceholder = 'Suchen...', children, onReset, activeCount = 0,
}) => (
  <div className="flex flex-nowrap items-center gap-2 bg-white dark:bg-slate-900 p-2 rounded-xl border dark:border-slate-800 shadow-xs overflow-x-auto no-scrollbar">
    {onSearch && (
      <div className="relative w-64 shrink-0">
        ...
        <Input placeholder={searchPlaceholder} value={search ?? ''} onChange={e => onSearch(e.target.value)} className="pl-9 h-9 text-sm" />
      </div>
    )}
