    ...
    <div className="flex flex-nowrap items-center gap-2 bg-white dark:bg-slate-900 p-2 rounded-xl border dark:border-slate-800 shadow-xs overflow-x-auto no-scrollbar">
      {onSearch && (
        <div className="relative w-64 shrink-0">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none dark:text-gray-400" size={16} />
      ...
      {activeCount > 0 && onReset && (
        <button type="button" onClick={onReset}
          className="flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 px-2 py-2 transition-colors whitespace-nowrap shrink-0 border border-transparent hover:border-red-500/20 rounded-lg"
